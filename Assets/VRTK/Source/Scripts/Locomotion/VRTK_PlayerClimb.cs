    ...
    public class VRTK_PlayerClimb : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            if (isClimbing)
            {
                ...
                //攀爬时的每一帧，playArea.position被更新为【被抓点的世界坐标（随原物更新）】+【玩家离被抓点的相对距离】-【controllerLocalOffset】
                //应该再加一个碰撞偏移
        ...
        protected virtual void Ungrab(bool carryMomentum, VRTK_ControllerReference controllerReference, GameObject target)
        {
            ...
            if (IsBodyColliding() && !IsHeadsetColliding())//身体在碰撞且头部在碰撞的话//但这些都是在Ungrab中才判定的
            {
                bodyPhysics.ForceSnapToFloor();//强制？？
            }
        ...
        protected virtual bool IsBodyColliding()
        {
            ...
            //bodyPhysics不为空且bodyPhysics.GetCurrentCollidingObject不为空，则判定为正在碰撞
            //
        }
