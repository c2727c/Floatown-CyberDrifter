...
HeldObject = TargetObject;//这里获得到被抓物！！！
...
HeldObject.GetComponent<Rigidbody>().AddForce(InteractorVelocity + Vector3.Cross(InteractorAngularVelocity,
RadiusVector), ForceMode.VelocityChange);

HeldObject.GetComponent<Rigidbody>().AddTorque(InteractorAngularVelocity
/ (1 + RadiusVector.magnitude), ForceMode.VelocityChange);
...
XRRigRigidbody.AddForce(-XRRigRigidbody.velocity, ForceMode.VelocityChange);
// zero the velocity

XRRigRigidbody.AddForce(-InteractorVelocity, ForceMode.VelocityChange);
// fling
...
Vector3 FrameInteractorVelocity = (Rigidbody.position - PrevPosition) /
Time.fixedDeltaTime;

Quaternion VelocityDiff = (Rigidbody.rotation * Quaternion.Inverse(PrevRotation));

Vector3 FrameInteractorAngularVelocity = (new Vector3(Mathf.DeltaAngle(0,
VelocityDiff.eulerAngles.x), Mathf.DeltaAngle(0,
VelocityDiff.eulerAngles.y), Mathf.DeltaAngle(0,
VelocityDiff.eulerAngles.z))
