...
Shader "Sci-Fi/SSFS/Simple Hologram"
{
	...
	half4 noise(float2 uv) { return -1.0 + 2.0 * tex2Dlod(_Noise, float4(uv,0.0,0.0)); }
	...
	SubShader
	{
		...
		Pass//red
		{
			...
			{ return col(i , fixed3(1.0,0.0,0.0)) * backfacedim(facing); }
			ENDCG
		}
		...
		Pass//blue
		{
			...
			{ return col(i , fixed3(0.0,0.0,1.0)) * backfacedim(facing); }
			ENDCG
		}
		...
		Pass//green
		{
			...
			{ return col(i , fixed3(0.0,1.0,0.0)) * backfacedim(facing); }
			ENDCG
		}
	}
}
